...
function writeSidebarCookie(open: boolean): void {
  ...
  // biome-ignore lint/suspicious/noDocumentCookie: simple UI-preference cookie; switching to cookies-next adds a dependency for a 1-line client-side write
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  // Derive a stable pseudo-random width from React.useId so the value
  // is identical on the server and client — Math.random would produce
  // different widths across that boundary and trigger hydration errors.
  const id = React.useId();
  const widthBucket = React.useMemo(() => {
    let hash = 0;
    for (let i = 0; i < id.length; i += 1) {
      // biome-ignore lint/suspicious/noBitwiseOperators: intentional 32-bit hash truncation — `| 0` forces int32 to keep hash bounded for % arithmetic
      hash = (hash * 31 + id.charCodeAt(i)) | 0;
    }
    return Math.abs(hash) % 40;
  }, [id]);
  const width = `${widthBucket + 50}%`;
