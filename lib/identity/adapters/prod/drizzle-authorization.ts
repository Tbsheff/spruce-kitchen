...
import { permission, rolePermission } from "@/lib/db/schema.ts";
import type {
  AuthorizationStore,
  Role,
  RoleAuthorization,
...
export function DrizzleAuthorizationStore(
  opts: { requestCache?: Map<Role, Promise<RoleAuthorization>> } = {}
): AuthorizationStore {
  const cache =
    opts.requestCache ?? new Map<Role, Promise<RoleAuthorization>>();
