...
const SEED_DATA = {
  permissions: [
    ...
    { id: "user:write", name: "Write Users", description: "Create and update users", resource: "user", action: "write" },
    { id: "user:delete", name: "Delete Users", description: "Delete user accounts", resource: "user", action: "delete" },
    { id: "mealplan:read", name: "Read Meal Plans", description: "View meal plans", resource: "mealplan", action: "read" },
    { id: "mealplan:write", name: "Write Meal Plans", description: "Create and update meal plans", resource: "mealplan", action: "write" },
    { id: "mealplan:delete", name: "Delete Meal Plans", description: "Delete meal plans", resource: "mealplan", action: "delete" },
    { id: "order:read", name: "Read Orders", description: "View orders", resource: "order", action: "read" },
    { id: "order:write", name: "Write Orders", description: "Create and update orders", resource: "order", action: "write" },
    { id: "order:delete", name: "Delete Orders", description: "Delete orders", resource: "order", action: "delete" },
    { id: "admin:users", name: "Manage Users", description: "Full user management", resource: "admin", action: "users" },
    { id: "admin:system", name: "System Admin", description: "System configuration", resource: "admin", action: "system" },
  ],
...
export async function seedDatabase() {
  ...
  try {
    ...
    await dbInstance.delete(auditLog)
    await dbInstance.delete(rateLimitRecord)
    await dbInstance.delete(rolePermission)
    await dbInstance.delete(order)
    await dbInstance.delete(mealPlan)
    await dbInstance.delete(session)
    await dbInstance.delete(account)
    await dbInstance.delete(user)
    await dbInstance.delete(permission)

    // 2. Seed permissions
    console.log("🔐 Creating permissions...")
    for (const perm of SEED_DATA.permissions) {
      await dbInstance.insert(permission).values({
        ...perm,
        createdAt: new Date()
      })
    }
    ...
    for (const userData of SEED_DATA.users) {
      // Insert user
      await dbInstance.insert(user).values({
        ...userData,
        createdAt: new Date(),
        updatedAt: new Date()
      })
    ...
    for (const planData of SEED_DATA.mealPlans) {
      await dbInstance.insert(mealPlan).values({
        ...planData,
        createdAt: new Date(),
        updatedAt: new Date()
      })
    ...
    for (const orderData of SEED_DATA.orders) {
      await dbInstance.insert(order).values({
        ...orderData,
        createdAt: new Date(),
        updatedAt: new Date()
      })
    ...
    for (const log of auditLogs) {
      await dbInstance.insert(auditLog).values(log)
    }
