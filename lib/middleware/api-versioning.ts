...
export function withApiVersioning(request: NextRequest): NextResponse | null {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: "Please use a supported API version in the URL path (e.g., /api/v1/) or Accept-Version header"
        },
        { 
          status: 400,
    ...
    if (!request.nextUrl.pathname.includes("/v")) {
      ...
      newUrl.pathname = newUrl.pathname.replace("/api/", `/api/${CURRENT_VERSION}/`)
...
export function addVersionHeaders(response: NextResponse, version?: string) {
  const headers = getVersionHeaders(version as any || CURRENT_VERSION)
