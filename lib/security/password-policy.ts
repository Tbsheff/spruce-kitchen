...
const COMMON_PASSWORDS = new Set([
  ...
  "123123", "12345678", "qwerty123", "1q2w3e4r", "admin123", "password12"
])
...
export class PasswordPolicyValidator {
  ...
  validatePassword(
    ...
    if (this.config.prohibitCommonPasswords && this.isCommonPassword(password)) {
      errors.push("Password is too common. Please choose a more unique password")
      score -= 30
    }

    // Personal information check
    if (this.config.prohibitPersonalInfo && userInfo) {
   ...
   */
  private containsPersonalInfo(password: string, userInfo: { email?: string; name?: string }): boolean {
  ...
  private hasRepeatingPatterns(password: string): boolean {
    ...
    for (let len = 2; len <= password.length / 2; len++) {
      for (let i = 0; i <= password.length - len * 2; i++) {
        ...
        const next = password.substr(i + len, len)
        if (pattern === next) {
          return true
        }
      }
