...
import {
  ...
  rateLimitMiddleware,
  resetRateLimit,
} from "@/lib/security/rate-limiting.ts";
import { SimpleAuditService } from "@/lib/security/simple-audit.ts";
import { adminProcedure, superAdminProcedure } from "@/lib/trpc/procedures.ts";
...
export const adminRouter = createTRPCRouter({
  ...
  resetUserRateLimit: superAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        await resetRateLimit(input.identifier, input.action as RateLimitAction);
