...
import type { IdentityProvider, RawSession } from "@/lib/identity/core/ports"
...
function toRawSession(result: BetterAuthSession): RawSession | null {
  ...
  const role = typeof (user as unknown as { role?: unknown }).role === "string"
...
export function BetterAuthIdentityProvider(req: Request | { headers: Headers }): IdentityProvider {
  return {
    async getSession() {
